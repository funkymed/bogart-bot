...
function searchMovie(search: any) {
  const url = `https://www.omdbapi.com/?s=${escape(search.value)}&apikey=${
    process.env.OMB_API_KEY
  }&t=${Date.now()}`;
...
function makeEmbed(search: any, movies: any) {
  const embeds = [];
